...
if (
  ...
  console.error(
    colors.bgRed(
      "❌ FATAL ERROR: Firebase environment variables (FIREBASE_PROJECT_ID, FIREBASE_PRIVATE_KEY, FIREBASE_CLIENT_EMAIL) are not set."
    )
  );
  throw new Error(
    "One or more required Firebase environment variables are not set. Please check your .env file."
  );
}
...
export const initFirebaseAdmin = () => {
  if (!isInitialized) {
    try {
      ...
      console.log(
        colors.bgMagenta(
          `✅ Firebase Admin for [${process.env.FIREBASE_PROJECT_ID}] initialized successfully`
        )
      );
      isInitialized = true;
    } catch (error) {
      console.error(
        colors.bgRed("❌ Error initializing Firebase Admin:"),
        error
      );
    }
  }
