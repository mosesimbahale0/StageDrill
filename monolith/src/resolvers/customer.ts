...
export const customerResolvers = {
  Query: {
    getCustomer: async (_: any, { customerId }: any) => {
      try {
    ...
    getCustomerByUid: async (_: any, { uid }: any) => {
      try {
  ...
  Mutation: {
     ...
     */
    async createCustomer(this: any, _: any, { input }: any, retryCount = 0) {
      ...
      try {
        ...
        console.log(
          `Attempting to find or create customer with _uid: ${input._uid}`
        );
        customerDoc = await Customer.findOneAndUpdate(
          { _uid: input._uid },
          input, // Using the entire 'input' for customer update.
        ...
        if (!customerDoc) {
          ...
          console.error(
            `createCustomer: Failed to create or find customer for _uid: ${input._uid}. This is unexpected with upsert:true.`
          );
          ...
          console.log(
            `Customer operation successful for _uid: ${input._uid}. Customer ID: ${customerDoc._id}`
          );
        }
        ...
        console.log(
          `Attempting to find or create fund for account_id: ${customerDoc._id}`
        );
        fundDoc = await Funds.findOneAndUpdate(
          { _account_id: customerDoc._id }, // The query to find the fund using the resolved customer's ID
          {
            $set: {
        ...
        if (!fundDoc) {
          ...
          console.error(
            `createCustomer: Failed to create or find fund for account_id: ${customerDoc._id}. This is unexpected with upsert:true.`
          );
          // Handle as per your application's error strategy
        } else {
          console.log(
            `Fund operation successful for account_id: ${customerDoc._id}. Fund ID: ${fundDoc._id}. PayPal ID: ${fundDoc._paypal_id}`
          );
        }
        ...
        if (error.code === 11000 && retryCount < 3) {
          console.error(
            `createCustomer: Duplicate key error (attempt ${retryCount + 1}/3). Retrying...`,
            `Error Message: ${error.message}`, // Log the full error message
        ...
        console.error(
          "createCustomer: Unrecoverable error during customer or fund creation/update:",
    ...
    updateCustomer: async (_: any, { customerId, input }: any) => {
      try {
        ...
        const updatedCustomer = await Customer.findByIdAndUpdate(
          customerId,
          input, // Use 'input' here to update with the provided data
