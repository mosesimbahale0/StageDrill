...
export const likeResolvers = {
  Query: {
    likes: async (_, { pageSize = 20, after, filter, sort }) => {
      let query: any = {};
      if (filter) {
        if (filter._customer_id) query._customer_id = filter._customer_id;
        if (filter._product_id) query._product_id = filter._product_id;
      }
      if (after) {
        query._id = { $lt: after };
      }
    ...
    toggleLikeProduct: async (_, { productId, customerId }) => {
      ...
      if (!product) {
        logger.warn(
          ...
          "toggleLikeProduct failed: Product not found"
        );
      ...
      if (!customer) {
        logger.warn(
          { productId, customerId },
          "toggleLikeProduct failed: Customer not found"
        );
      ...
      if (existingLike) {
        ...
        try {
          ...
          if (error.code === 11000) {
            ...
            logger.warn(
              { productId, customerId },
              "Duplicate like attempt blocked by index."
            );
          } else {
            logger.error(
              { err: error, productId, customerId },
              "Error saving like"
            );
