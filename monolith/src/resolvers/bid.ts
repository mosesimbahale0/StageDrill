...
export const bidResolvers = {
  Query: {
    ...
    bidsByProduct: async (_, { productId }) =>
      Bid.find({ _product_id: productId }).populate("_customer_id"),
    bidsByCustomer: async (_, { customerId }) =>
      Bid.find({ _customer_id: customerId }).populate("_customer_id"),
  },
  Mutation: {
    // --- Bid Mutation ---
    createBid: async (_, { input }) => {
      ...
      if (!product) {
        logger.warn(
          ...
          "createBid failed: Product not found"
        );
      ...
      if (new Date() > new Date(product.auction_end)) {
        logger.warn(
          { productId: _product_id },
          "createBid failed: Auction has ended"
        );
      ...
      try {
        await session.withTransaction(async () => {
          ...
          if (
            !product.bidders.some((bidderId) => bidderId.equals(_customer_id))
          ) {
