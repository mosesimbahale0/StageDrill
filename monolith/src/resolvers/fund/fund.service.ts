...
import  Funds, { IFundSchema } from './fund.model.js'; // Adjust path as per your project structure

import { Fund, CreateFundInput, UpdateFundDataInput } from './fund.types.js';
...
const convertDocToFund = (doc: IFundSchema | null): Fund | null => {
  if (!doc) {
    return null;
  }
...
export const findFundById = async (fundId: string): Promise<Fund | null> => {
  try {
    // With a typed model, fundDoc will be IFundSchema | null
    const fundDoc: IFundSchema | null = await Funds.findById(fundId);
...
export const findFundByPaypalId = async (paypalId: string): Promise<Fund | null> => {
  try {
    const fundDoc: IFundSchema | null = await Funds.findOne({ _paypal_id: paypalId });
    return convertDocToFund(fundDoc);
  } catch (error) {
...
export const findFundByAccountId = async (accountId: string): Promise<Fund | null> => {
  try {
    const fundDoc: IFundSchema | null = await Funds.findOne({ _account_id: accountId });
    return convertDocToFund(fundDoc);
  } catch (error) {
...
export const createOrUpsertFund = async (
  input: CreateFundInput
): Promise<{ fund: Fund; wasCreated: boolean }> => {
  const { _account_id, _paypal_id, is_active, tokens } = input;

  try {
    const existingFundDoc: IFundSchema | null = await Funds.findOne({ _account_id });

    if (existingFundDoc) {
      const updatedFundDoc: IFundSchema | null = await Funds.findOneAndUpdate(
        { _account_id },
        { _paypal_id, is_active, tokens },
      ...
      if (!fundObject) throw new Error("Conversion to Fund object failed for updated fund.");
      return { fund: fundObject, wasCreated: false };
    } else {
      // Funds.create returns a fully typed IFundSchema document
      const newFundDoc: IFundSchema = await Funds.create({
        _account_id,
        _paypal_id,
        is_active,
        tokens,
      ...
      // The check below is mostly for type safety / unexpected nulls post-conversion
      if (!fundObject) throw new Error("Conversion to Fund object failed for new fund.");
...
export const updateFundInDB = async (
  fundId: string,
  fundData: UpdateFundDataInput
): Promise<Fund | null> => {
  try {
    const updatedFundDoc: IFundSchema | null = await Funds.findByIdAndUpdate(fundId, fundData, {
      new: true,
    });
...
export const deleteFundFromDB = async (fundId: string): Promise<Fund | null> => {
  try {
    const deletedFundDoc: IFundSchema | null = await Funds.findByIdAndDelete(fundId);
