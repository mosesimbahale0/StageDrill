...
async function getAccessToken() {
    ...
    // Fetch new token
    const auth = Buffer.from(`${process.env.PAYPAL_CLIENT_ID_SANDBOX}:${process.env.PAYPAL_CLIENT_SECRET_SANDBOX}`).toString("base64");
    try {
        const response = await fetch(`${process.env.PAYPAL_API_URL_SANDBOX}/v1/oauth2/token`, {
            method: "POST",
            headers: {
