...
const ioredisConfig = {
    ...
    port: typeof nodeRedisConfig.socket.port === "string"
...
export async function initRedis() {
    try {
        ...
        await redisClient.connect();
        console.log(colors.bgMagenta.bold("🔥 Connected to Upstash Redis (node-redis client)!"));
        // Sanity check with better error handling
        const testKey = "upstash_test_key";
        const testValue = "test_value_works";
        await redisClient.set(testKey, testValue);
        console.log(colors.green(`✅ Set "${testKey}" = "${testValue}" (node-redis)`));
        const retrievedValue = await redisClient.get(testKey);
        if (retrievedValue === testValue) {
            console.log(colors.cyan(`🔍 Retrieved "${testKey}" = "${retrievedValue}" (node-redis)`));
        }
        else {
            console.error(colors.red("🔴 Value mismatch - Connection verification failed (node-redis)"));
            throw new Error("Redis connection verification failed (node-redis)");
        }
    }
    catch (error) {
        console.error(colors.red.bold("❌ Error in initRedis (node-redis setup):"), error);
...
export async function testRedisConnection() {
    try {
        ...
        console.log(colors.bgGreen("✅ Temporary connection successful (node-redis to Upstash)!"));
        await tempClient.disconnect();
    }
    catch (error) {
        console.error(colors.red("❌ Temporary connection failed (node-redis to Upstash):"), error);
