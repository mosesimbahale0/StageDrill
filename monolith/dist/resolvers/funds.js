...
export const fundsResolvers = {
    Query: {
        ...
        getFundByPaypalId: async (_, { paypalId }) => {
            try {
        ...
        getFundByAccountId: async (_, { accountId }) => {
            try {
    ...
    Mutation: {
        createFund: async (_, { _paypal_id, _account_id, is_active, tokens }, { pubsub } // Assuming pubsub is passed in context
        ) => {
            ...
            try {
                ...
                if (existingFundByAccountId) {
                    ...
                    const updatedFund = await Funds.findOneAndUpdate({ _account_id: _account_id }, { _paypal_id, is_active, tokens }, // Update _paypal_id, is_active, tokens
                ...
                if (error.code === 11000 && error.keyPattern) {
                    if (error.keyPattern._account_id) {
                        ...
                        throw new Error(`Account ID ${_account_id} already exists or a conflict occurred.`);
                    }
                    ...
                    if (error.keyPattern._paypal_id) {
                        throw new Error(`CRITICAL: Unique constraint violation on _paypal_id. The unique index on the _paypal_id field MUST be removed from the 'funds' collection in MongoDB.`);
                    }
        ...
        updateFund: async (_, { fundId, fund }) => {
            try {
