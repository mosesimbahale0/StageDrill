...
async function handlePaymentCaptureCompleted(resource, retryCount = 0) {
    if (resource.resource_type !== "checkout-order")
        return;
    ...
    try {
        ...
        const fund = await Funds.findOneAndUpdate({ _account_id: transaction._account_id }, updateOps, {
            upsert: true,
            new: true,
            runValidators: true,
        });
        ...
        console.log(`Capture completed: payer=${payerId}, tx=${transactionId}, tokens=${tokens}`);
    }
    catch (error) {
...
async function handleCheckoutOrderApproved(resource) {
    const pu = resource.purchase_units[0];
    ...
    if (!user)
        return console.error("User not found:", payerId);
    ...
    pubsub.publish("NOTIFICATION_CREATED", { notificationCreated: note });
    console.log(`Order approved: payer=${payerId}, tx=${resource.id}, tokens=${tokens}`);
}
export const paypalResolvers = {
    Query: {
        getTransactionByPaypalId: async (_, { paypalId }) => {
            try {
                const transaction = await Transaction.findOne({
                    _paypal_id: paypalId,
                });
    ...
    Mutation: {
        ...
        // PAYPAL RECORDS
        //---------------------------------------------
        createUserPaypalRecords: async (_, { input, }) => {
            try {
                ...
                const existingPaypalId = existingCustomer._paypal_ids.find((paypalId) => paypalId._paypal_id === notification._paypal_id);
                if (!existingPaypalId) {
