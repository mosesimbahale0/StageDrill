...
async function recalculateCustomerRating(customerId) {
    ...
    try {
        ...
        logger.info({ customerId, review_count, average_rating }, `Recalculated ratings for customer`);
    }
    catch (error) {
...
export const reviewResolvers = {
    Query: {
        ...
        reviewsByProduct: async (_, { productId }) => {
            ...
            logger.warn(`reviewsByProduct query called for ${productId}, but this is not supported by the DB schema.`);
            return [];
        },
    },
    Mutation: {
