...
export const templateResolvers = {
    Query: {
        getAllTemplates: async (_, { skip = 0, limit = 10 }) => {
            try {
        ...
        //--------------------------------------------------
        getTemplateById: async (_, { templateId }) => {
            try {
        ...
        semanticSearch: async (_, { query, top_k }) => {
            try {
                // Set defaults for optional parameters
                top_k = top_k || 50;
                // Combine alpha with query and top_k
                // Send request to hybrid search service
                const response = await fetch("http://localhost:8003/api/v1/hybrid-search", {
                    method: "POST",
                ...
                if (!response.ok) {
                    ...
                    throw new Error(`Hybrid search request failed: ${response.status} - ${errorBody}`);
                }
                ...
                const templateMap = new Map(templatesFromDB.map((template) => [template._id.toString(), template]));
        ...
        hybridSearch: async (_, { query, top_k }) => {
            try {
                // Set defaults for optional parameters
                top_k = top_k || 50;
                // Combine alpha with query and top_k
    ...
    Mutation: {
        // Clone Template
        //---------------------------------------------------
        cloneTemplate: async (_, { _account_id, _template_id, characters, name, description }) => {
            try {
                ...
                await Templates.findByIdAndUpdate(_template_id, { $push: { cloners: { _account_id: _account_id } } }, { new: true });
                return funspot;
            }
            catch (error) {
                throw error;
        ...
        updateTemplate: async (_, { templateId, input }) => {
            try {
        ...
        deleteTemplate: async (_, { templateId }) => {
            try {
        ...
        combinedTemplateLikingLogic: async (_, { customerId, templateId }) => {
            try {
                const template = await Templates.findById(templateId);
                ...
                const isLiked = customer.likes.some((like) => like._template_id === input._template_id);
                let updatedTemplate;
                let updatedCustomer;
                if (!isLiked) {
                    // Like the template
                    updatedTemplate = await Templates.findByIdAndUpdate(templateId, { $push: { liking: input2 } }, { new: true });
                    updatedCustomer = await Customer.findByIdAndUpdate(customerId, { $push: { likes: input } }, { new: true });
                }
                else {
                    // Unlike the template
                    updatedTemplate = await Templates.findByIdAndUpdate(templateId, { $pull: { liking: input2 } }, { new: true });
                    updatedCustomer = await Customer.findByIdAndUpdate(customerId, { $pull: { likes: input } }, { new: true });
                }
        ...
        ratingTemplateLogic: async (_, { customerId, templateId, stars, text }) => {
            try {
                ...
                const template = await Templates.findById(templateId).populate("rating");
                if (!template) {
                    throw new Error("Template not found");
                }
                const customer = await Customer.findById(customerId);
                ...
                const existingReviewIndex = template.rating.findIndex((review) => review._account_id.toString() === customerId);
                let updatedTemplate;
        ...
        deleteRatingByCustomerId: async (_, { templateId, customerId }) => {
            try {
                const template = await Templates.findById(templateId);
                if (!template) {
                    throw new Error("Template not found");
                }
                const reviewIndex = template.rating.findIndex((review) => review._account_id === customerId);
                if (reviewIndex !== -1) {
