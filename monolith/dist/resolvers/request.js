...
export const requestResolvers = {
    Query: {
        getRequestById: async (_, { _request_id }) => {
            try {
        ...
        getAllRequestsByFunspotId: async (_, { funspotId }) => {
            try {
    ...
    Mutation: {
        ...
        mediateRequest: async (_, { input }) => {
            try {
                ...
                const requestsHistory = await Request.find({
                    ...
                    .limit(5);
                // Step 2 get the responses for each request
                const responsesHistory = await Promise.all(requestsHistory.map(async (request) => {
                    const responses = await Cleanresponse.find({
                        _request_id: request._id,
                    });
                    return responses;
        ...
        Based on the user's input "${input.text}" and the context provided, generate a response using the most relevant personas.
        `;
                // Log all the data being sent
                console.log(colors.bgBlue("🚀  Data sent to FastAPI:"));
                console.log(colors.bgBlue("------------------------------------"));
                console.log(colors.bgBlue("Human Message: " + prompt));
                console.log(colors.bgBlue("Context: " + JSON.stringify(context)));
                console.log(colors.bgBlue("Personas: " + JSON.stringify(charactersWithDetails)));
                console.log(colors.bgBlue("User: " + JSON.stringify(user)));
                console.log(colors.bgBlue("Funspot Summary: " + funspotSummary));
                console.log(colors.bgBlue("Instructions: " + instructions));
                console.log(colors.bgBlue("------------------------------------"));
                ...
                eventSource.onmessage = async (event) => {
                    try {
                        const response = JSON.parse(event.data);
                        console.log("✅ Received response from character:", response.character_index);
                        // Validate and store the response
                        if (response &&
                            response.character_index !== undefined &&
                            response.text) {
                            ...
                            console.log("✅ Response successfully stored in the database:", response.text);
                            // // Deduct 1 token from the user's funds
                            // try {
                            ...
                            try {
                                ...
                                if (!existingFund) {
                                    ...
                                    const updatedFund = await Funds.findOneAndUpdate({ _account_id: input._account_id }, {
                                        tokens: (() => {
                                            const currentTokens = parseFloat(existingFund.tokens);
                                            if (isNaN(currentTokens)) {
                                                console.error("Invalid existing tokens value:", existingFund.tokens);
                                                return existingFund.tokens; // Or handle the error differently
                                            }
                                            return (currentTokens - 1).toString(); // Deduct 1 token
                                        })(),
                                    ...
                                    if (updatedFund) {
                                        console.log("💰💰💰Tokens deducted successfully. Remaining tokens:", updatedFund.tokens);
                                    }
                                    else {
                                        console.error("Failed to update fund after deducting tokens.");
                                    }
                                }
                            }
                            catch (error) {
                            ...
                            // If all characters have responded, close the SSE connection
                            if (responseCount === totalCharacters) {
                                console.log("✅ All characters have responded. Closing SSE connection.");
                                eventSource.close(); // Close the connection once all responses are received
                            }
                        }
                        else {
        ...
        deleteRequest: async (_, { _request_id }) => {
            try {
